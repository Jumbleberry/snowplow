...
{% macro snowplow_web_events_details() %}

    {{ adapter_macro('snowplow.snowplow_web_events_details') }}

{% endmacro %}


{% macro default__snowplow_web_events_details() %}

-- transform all *new* events, then delete existing
-- and insert new (by event_id). No sql_where because
...
prep as (
    ...
    where ev.platform = 'web'
      and ev.event_name  in ('page_view', 'page_ping')
